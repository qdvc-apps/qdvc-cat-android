 ...
 * (Adapted from qdvc-markdown-notebook-android's SystemFonts.)
 */
...
/**
 * A loaded custom font built from whichever variant files the user supplied.
 * The [fontFamily] contains the present style variants, so the highlighter's
 * bold / italic spans (where a grammar uses them) render in the matching face.
...
object SystemFonts {
    ...
    /**
     * Returns the list of usable device fonts, de-duplicated by display name
     * and sorted alphabetically. Any file that fails to load is skipped. Pure
     * I/O — safe to call off the main thread.
     */
    ...
    // ---- Custom font: up to 4 files copied into app storage ----
    //
    // Exactly one file per variant slot, at a fixed path. Re-picking a slot
    // overwrites its file; clearing deletes it. Because the files live in the
    // app's own storage, nothing external can go missing between sessions.

    private fun customDir(context: Context): File =
        File(context.filesDir, "custom_fonts").apply { mkdirs() }

    private fun slotFile(context: Context, variant: FontVariant): File =
        File(customDir(context), "${variant.name.lowercase()}.ttf")

    /**
     * Copies the font at [uri] into the fixed slot for [variant], replacing any
     * existing file. Returns the original file name on success, or null if the
     * file can't be read or isn't a usable font.
     */
    fun copyIntoSlot(context: Context, uri: Uri, variant: FontVariant): String? {
        return try {
            ...
            val target = slotFile(context, variant)
    ...
    fun clearSlot(context: Context, variant: FontVariant) {
        val target = slotFile(context, variant)
        target.delete()
    ...
    fun slotName(context: Context, variant: FontVariant): String? {
        val target = slotFile(context, variant)
    ...
    /** Builds a [CustomFont] from whichever slot files exist, or null if none. */
    fun loadCustomFont(context: Context): CustomFont? {
        val fonts = mutableListOf<Font>()
        val names = mutableMapOf<FontVariant, String>()
        for (variant in FontVariant.entries) {
            val file = slotFile(context, variant)
            if (!file.exists()) continue
            val (weight, style) = variant.weightAndStyle()
            try {
                fonts.add(Font(file, weight = weight, style = style))
                names[variant] = slotName(context, variant) ?: variant.label
            } catch (e: Exception) {
